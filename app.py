...
st.markdown(
    ...
    <div class="hero">
        <h1>🛡️ VoiceGuard</h1>
        <p>AI-Powered Voice Deepfake & Spoof Detection System</p>
        <p>
            Analyze uploaded audio, record your voice, or perform
...
def get_ai_score(predictions):
    """
    Convert the model's output into one AI/spoof probability.

    The model currently returns labels such as:
    real / fake
    """
...
def analyze_audio(audio_bytes, suffix=".wav"):
    ...
    try:
        ...
        # IMPORTANT:
        # Load audio with librosa instead of letting the
        # Transformers pipeline use PyAV directly.
        # This avoids the macOS duplicate FFmpeg/PyAV warning.
        ...
        # More robust than relying on only one audio segment.
        final_score = (
            0.55 * max_score
            + 0.30 * top_mean
            + 0.15 * mean_score
...
def get_verdict(score):
    if score >= 0.85:
        return "AI GENERATED / SPOOF", "HIGH"

    if score >= 0.50:
        return "SUSPICIOUS / POSSIBLY AI", "MEDIUM"
...
def show_result(result):
    ...
    for i, chunk_score in enumerate(
        ...
        st.write(
            f"Chunk {i}: "
            f"**{chunk_score * 100:.1f}% AI score**"
        )

        st.progress(
            min(max(chunk_score, 0.0), 1.0)
        )
...
class LiveAudioProcessor(AudioProcessorBase):
    ...
    def recv(self, frame):
        try:
            ...
            # Audio callback must never crash the stream.
            pass
...
# =========================================================
    ...
    if ctx.state.playing:
        ...
        st.success("🟢 Live microphone active")
        ...
        while ctx.state.playing:
            ...
            if live_scores:
                ...
                if current_score >= 0.85:
                    ...
                    verdict = "🟠 SUSPICIOUS / POSSIBLY AI"
                    risk = "MEDIUM"
